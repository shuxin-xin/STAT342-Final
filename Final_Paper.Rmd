...
Then, by distributive laws, $P(I)$, the probability of infections, is: $P(I)= P(I \cap V) + P(I \cap V^c)= \frac{n_1 \pi_v + n_2 \pi_p}{n_1 + n_2}$ (See full process in Appendix 8.2). 
...
$\psi = 1 - \frac{\pi_v}{\pi_p} = 1 - \frac{\pi}{1-\pi} = \frac{1-2\pi}{1-\pi}$. At the same time, we also know that $\pi = \frac{1 - \psi}{2 - \psi}$. (See process in Appendix 8.3)
...
\end{align*}
Here, to maximize $\psi$, let's start with the first derivative:
\begin{align*}
...
\psi &= \frac{170 - 2t}{170 - t} \qquad \text{(See full process in Appendix 8.4)}
\end{align*}
...
Then, we did a likelihood ratio test on $\psi$. Notice that here our alternative hypothesis becomes $H_1:\psi_0\neq 0.3$ because  Plug in $\psi_0^{mle}=0.9506173$ and $t=8$, we know that the likelihood ratio test statistic $W$ equals to: $W = 2[\ell(\widehat{\psi_0^{mle}}) - \ell(\psi^{null})]=121.6012$. (See full process in Appendix 8.5)
...
```{r label="graph", echo=F,fig.height=3, fig.width=9.5}
p_a <- ggplot()+
...
```{r echo = FALSE, warning=F,fig.height=3, fig.width=9.5}
p1+p2
```
...
## Detailed Calculation of $P(I)$ for 4.1
\begin{align*}
...
## Representing $\psi$ using $\pi$ and representing $\pi$ using $\psi$ for 4.1
...
## Detailed Calculation of the first derivative of $\ell(\psi)$ for 4.3.1
\begin{align*}
\frac{d}{d \psi} \ell(\psi) &= \frac{d}{d \psi} \left(ln\left(\binom{170}{t}\right) + t \cdot (ln(1 - \psi) - ln(2 - \psi)) + (t - 170) \cdot ln(2 - \psi)\right)=0 \\
...
## Detailed Calculation of W for 4.3.3
\begin{align*}
