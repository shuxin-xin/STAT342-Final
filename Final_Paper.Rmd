...
COVID 19 has afflicted tens of millions of people around the world. In this paper, we utilize the data from the article "Safety and Efficacy of the BNT162b2 mRNA COVID 19 Vaccine", aiming to figure out whether the BNT162b2 vaccine pass the FDA's requirement that the new vaccine should be at least 30\% efficacy using three methods: Maximum Likelihood, Bayesian, and Bootstrapping. The results from the three approaches are consistent that BNT162b2 was 95% effective in preventing COVID 19 (95% confidence interval in Maximum Likelihood, 91.6% to 98.6%; 95% credible interval in Baysian, 90.3% to 97.6% for prior Beta(0.700102, 1); 95% credible interval in Baysian, 90.1% to 97.5% for prior Beta(1, 1); 95% confidence interval in Bootstrapping, 90.9% to 98.2%). Then, we conclude that the true vaccine efficacy is greater than 30% and with overlapped 95% interval [0.9155627, 0.9750466].
...
$\psi = 1 - \frac{\pi_v}{\pi_p} = \frac{\pi_p - \pi_v}{\pi_p}$. Based on the data from table 1 above, we can easily calculated that $\pi_v = \frac{8}{17411}$, $\pi_p = \frac{162}{17511}$, and $\hat{\psi} = \frac{\pi_p - \pi_v}{\pi_p}=0.9503337$. That means the vaccine BNT162b2 can reduce risk of having COVID 19 by around 95\% compare to placebo.
...
Let $S$ denote the total cases in two groups where $S=X+Y$ and let $T$ denotes the number of cases in vaccine group from $S$ total cases. Let $V$ denote the event that "being assigned to BNT162b2 group" and $I$ denote the event that "infections".

Then, $P(V)$, the probability of subjects being assigned to the BNT162b2 group, is $\frac{n_1}{n_1+n_2}$. With rule of complement, $P(V^c)$, the probability of subjects being assigned to the placebo group, is $\frac{n_2}{n_1+n_2}$.

Then, by distributive laws, $P(I)$, the probability of infections, is: $P(I)= P(I \cap V) + P(I \cap V^c)= \frac{n_1 \pi_v + n_2 \pi_p}{n_1 + n_2}$ (See full process in Appendix 8.1). 

Let $\pi$ denote the probability of a subject with the BNT162b2 vaccine but in the infection group, that is $\pi = P(V|I)=P(\text{BNT162b2 | COVID 19})$. Since participants underwent randomization in the paper, every case is independent of each other. With independence and a fixed probability of $\pi$, a reasonable model that we can use for $T$ is $T \sim Binom(s, \pi)$ where $\pi = P(V|I)$. Now, let's find $\pi$:
...
At the same time, we also know that $\pi = \frac{1 - \psi}{2 - \psi}$. (See process in Appendix 8.2)

Based on the data from table 1, we can easily calculated that $s=x+y=8+162=170$ and $\pi=\frac{\pi_v}{\pi_v+\pi_p}=0.04731632$. Then, $T \sim Binom(s=170, \pi=0.04731632)$. And we will utilize these approach in the Maximum Likelihood and Bayesian approach.
...
We assume our significance level is 0.05.
...
\end{align*}
From Table 1, given that 8 cases in Vaccine group, we know that $t=8$, then $\psi = \frac{170 - 2\cdot 8}{170 - 8}=0.9506173$. Then, $\ell''(\psi)=\frac{-8\cdot (2 - 0.9506173)^{2} + 170 \cdot (1 - 0.9506173)^{2}}{(1 - 0.9506173)^{2} \cdot (2 - 0.9506173)^{2}}=-3126.12565702$.

Since we only have one critical point and the second derivative of $\ell(\psi)$ with respect to $\psi$ on that critical point is negative, we can say that the function $\ell(\psi)$ is always concave down and the critical point, $\psi = \frac{170 - 2t}{170 - t}=0.9506173$ is the global maximum of $\ell(\psi)$. Therefore, $\widehat{\psi_0^{mle}} = 0.9506173$ is a MLE for $\psi$. 
...
Then, we did a likelihood ratio test on $\psi$. Notice that here our alternative hypothesis becomes $H_1:\psi_0\neq 0.3$ because  Plug in $\psi_0^{mle}=0.9506173$ and $t=8$, we know that the likelihood ratio test statistic $W$ equals to: $W = 2[\ell(\widehat{\psi_0^{mle}}) - \ell(\psi^{null})]=121.6012$. (See full process in Appendix 8.4)
...
Assuming $H_0$ is true, the P-value is the $P(W > 121.6012)$. We only consider $W > 121.6012$ because if the ratio $\frac{L(\widehat{\psi_0}^{mle})}{L(\widehat{\psi_0}^{null})}$ equals to 1, this equation illustrates exactly the null value. And also, the larger the values of the ratio $\frac{L(\widehat{\psi_0}^{mle})}{L(\widehat{\psi_0}^{null})}$ yield larger values of $W$ and provide stronger evidence against $H_0$. Since large values of W provide evidence in the direction of the alternative, the P-value is the right tail probability from the $\chi^2_1$ distribution. Thus, using R, the P-value for the Likelihood Ratio Test statistic using the approximate chi square distribution is $P(W > 121.6012) = P(\psi > 0.3) = 2.822313 \times 10^{-28}$.
...
### Calculatiion of the Posterior Distribution
...
We also plot the prior and posterior distributions for both of the approaches to see the difference:

```{r label="graph", echo=F,fig.height=4, fig.width=9.5}
p_a <- ggplot()+
...
For prior Beta(0.700102, 1), $P(\psi_0 \leq 0.3|t)=1.96 \times 10^{-28}$.

For prior Beta(1, 1), $P(\psi_0 \leq 0.3|t)=3.73 \times 10^{-28}$.
...
\end{table}
  ...
  The results from all of the approaches are consistent that we have enough evidence to reject the null hypothesis and claim the true efficiency of the vaccine is larger than 30\%. We also have strong confidence that the BNT162b2 vaccine efficacy is larger than 90\%.
  
# Discussion/Conclusion

## Conclusion

  The 95\% Confidence Intervals of the true vaccine efficacy using the Maximum Likelihood is [0.9155627, 0.9856719]. That is, we are 95% confident that the true value of the vaccine efficacy is between 0.9155627 and 0.9856719. The 95\% Confidence Intervals of the true vaccine efficacy using the Bootstrapping is [0.9097410, 0.9819328]. That is, we are 95% confident that the true value of the vaccine efficacy is between 0.9097410 and 0.9819328. The 95\% Credible Intervals of the true vaccine efficacy for Bayesian prior Beta(0.700102, 1) and Bayesian Prior Beta(1, 1) are [0.9035199, 0.9762552] and [0.9009794, 0.9750466] respectively. The 95\% Bayesian credible interval [0.9035199, 0.9762552] suggests that the middle 95% of the posterior distribution of vaccine efficacy resulting from the prior $Beta(0.700102, 1)$ is the range 90.3% to 97.6%. Thus, there is a 95% probability that the true vaccine efficacy is between 90.3% and 97.6%. The 95\% Bayesian credible interval [0.9009794, 0.9750466], suggesting that the middle 95% of the posterior distribution of vaccine efficacy resulting from the prior $Beta(1,1)$ is the range 90.3% to 97.6%. Thus, there is a 95% probability that the true vaccine efficacy in between 90.1% and 97.5%.
...
## Representing $\psi$ using $\pi$ and representing $\pi$ using $\psi$ for 4.1.2
...
## Detailed Calculation of the first derivative of $\ell(\psi)$ for 4.2
\begin{align*}
\frac{d}{d \psi} \ell(\psi) &= \frac{d}{d \psi} \left(ln\left(\binom{170}{t}\right) + t \cdot (ln(1 - \psi) - ln(2 - \psi)) + (t - 170) \cdot ln(2 - \psi)\right)=0 \\
...
## Detailed Calculation of W for 4.2
\begin{align*}
...
## Detailed Calculation of $\alpha$ for 4.3.1
...
## Code for Likelihood for 4.2
```{r ref.label="likelihood", echo = TRUE, eval = F}

```

## Code for Bayesian for 4.3
