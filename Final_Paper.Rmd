...
Based on the given information in Chapter 25, we have the estimated variance of the MLE of $\psi$ is $-\frac{1}{\ell''(\widehat{\psi_0^{mle}})}$

\begin{align*}
Var[\widehat{\psi_0^{mle}}] &= -\frac{1}{\ell''(\widehat{\psi_0^{mle}})} \\
&= -\frac{1}{\frac{-8}{(1 - 0.9506173)^{2}} + \frac{170}{(2 - 0.9506173)^{2}}} \\
&= -\frac{1}{\frac{-8}{0.002438651} + \frac{170}{1.101204}} \\
&= -\frac{1}{-3280.502 + 154.3765} \\
&= 0.0003198848 \\
SD[\widehat{\psi_0^{mle}}] &= \sqrt{Var[\widehat{\psi_0^{mle}}]} \\
&= 0.01788532 \\
\end{align*}
...
\begin{align*}
\widehat{\psi_0^{mle}} &\pm Z \cdot \widehat{SE} \\
\widehat{\psi_0^{mle}} &\pm Z \cdot \sqrt{-\frac{1}{\ell''(\widehat{\psi_0^{mle}})}} \\
0.9506173 &\pm 1.96 \cdot 0.01788532 \\
[0.9155621, & \: 0.9856725] \\
\end{align*}

Because the null hypothesis is $\psi^{null} = \frac{1 - 2\pi}{1 - \pi} = 0.3$ and $\widehat{\psi_0^{mle}} = 0.9506173$.

\begin{align*}
W &= 2[\ell(\widehat{\psi_0^{mle}}) - \ell(\psi^{null})] \\
&= 2[ln\left(\binom{170}{t}\right) + t \cdot (ln(1 - \widehat{\psi_0^{mle}}) - ln(2 - \widehat{\psi_0^{mle}})) + (t - 170) \cdot ln(2 - \widehat{\psi_0^{mle}}) - \\
& \quad (ln\left(\binom{170}{t}\right) + t \cdot (ln(1 - \psi^{null}) - ln(2 - \psi^{null})) + (t - 170) \cdot ln(2 - \psi^{null}))] \\
&= 2[t \cdot ((ln(1 - \widehat{\psi_0^{mle}}) - ln(2 - \widehat{\psi_0^{mle}})) - (ln(1 - \psi^{null}) + ln(2 - \psi^{null}))) \\
& \quad + (t - 170) \cdot (ln(2 - \widehat{\psi_0^{mle}})-ln(2 - \psi^{null}))] \\
...
Since large level of $W$ provide evidence in the direction of alternative, the p-value is the right tail probability from the chisquare distribution.

```{r label="likelihood", include = FALSE, cache = TRUE}
pchisq(q = 106.5508, df = 1, lower.tail = F)
```

Because at $t = 8$, we have $W = 106.5508$, and because the P-value for $P(W < 106.5508) = 5.584252\times 10^{-25}$, which is very small. So, we have enough evidence to reject the null hypothesis and we have $P(\psi > 0.3) = 1$. Therefore, we can conclude that the vaccine efficacy is greater than 30\%.
...
```{r label="method1", include = FALSE, cache = TRUE, warning = F}
n <- 170
t <- 8
...
For the likelihood method, we calculate the p-value $P(W > 106.5508) = 1$, therefore,
