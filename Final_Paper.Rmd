...
Then, by distributive laws, $P(I)$, the probability of infections, is: $P(I)= \frac{n_1 \pi_v + n_2 \pi_p}{n_1 + n_2}$ (See process in Appendix 8.). 
...
At the same time, we also know that $\pi = \frac{1 - \psi}{2 - \psi}$. (See process in Appendix 8.1)
...
\psi &= \frac{170 - 2t}{170 - t} \qquad \text{(See full process in Appendix 8.2)}
\end{align*}
...
Then, we did a likelihood ratio test on $\psi$. Notice that here our alternative hypothesis becomes $H_1:\psi_0\neq 0.3$ because  Plug in $\psi_0^{mle}=0.9506173$ and $t=8$, we know that the likelihood ratio test statistic $W$ equals to: $W = 2[\ell(\widehat{\psi_0^{mle}}) - \ell(\psi^{null})]=121.6012$. (See process in Appendix 8.3)
...
Bayesian Prior Beta(1, 1) (CredI) & 0.9009794 & 0.9750466 & 3.73$10^{-28}$ \\ \hline
...
# Discussion/Conclusion
  ...
  Under regularity conditions, the likelihood estimates converge to the true parameter values as the sample size increases. This property ensures that the likelihood method produces consistent estimates. Bayesian analysis uses prior beliefs or evidences for the parameters of interest. Compared to other two methods, the likelihood method is more efficient in terms of statistical power, because it can provide precise estimates of unknown parameters when the sample size is large. However, the likelihood method relies on assumptions about the data distribution. If we assume distribution inaccurately, the estimates obtained from the likelihood method may be biased or inefficient. 
   Thus, we have enough evidence to reject the null hypothesis and conclude that the true vaccine efficacy is greater than 30\%.
  
   The 95\% Bayesian credible interval [0.9035199, 0.9762552] suggests that for $\psi$ represents the vaccine efficacy against COVID 19, the middle 95% of the posterior distribution of vaccine efficacy resulting from the prior $Beta(0.700102, 1)$ is the range 90.3% to 97.6%. There is a 95% probability that $\psi_0$ in between 90.3% and 97.6%. The 95\% Bayesian credible interval [0.9009794, 0.9750466] suggests that for $\psi$ represents the vaccine efficacy against COVID 19, the middle 95% of the posterior distribution of vaccine efficacy resulting from the prior $Beta(1,1)$ is the range 90.3% to 97.6%. There is a 95% probability that $\psi_0$ in between 90.1% and 97.5%.
   
  Since posterior probability $P(\psi_0 \leq 0.3|t)$ for $Beta(1,1)$ is greater than that of $Beta(0.700102, 1)$ ($3.73 \times 10^{-28} > 1.96 \times 10^{-28}$), then $1 - 3.73 \times 10^{-28} < 1 - 1.96 \times 10^{-28}$. Thus, $P(\psi_0 > 0.3|t)$ for prior Beta(0.700102, 1) is much closer to 1, so prior Beta(0.700102, 1) has greater evidence to reject the null hypothesis that vaccine efficacy is 30%. The probability that the true vaccine efficacy against COVID 19 is greater than 30% is approximately 100%. The evidence suggests that vaccine efficacy is greater than 30%.

  Then, we can have posterior distribution based on observed data and prior. It also allows for the direct estimation of functions of parameters by using the posterior distribution instead of plugging the estimated parameters into the functions. Bayesian gives interpretable results of probabilities of the parameters fall in credible intervals, knowing the uncertainty of the parameter directly. However, it is hard to find the prior. Prior plays an important role in results. Posterior distributions are easily affected by choices of prior. If we do not choose a proper prior, the result will be misleading. 
  ...
  In conclusion, the findings from the three analysis methods (Maximum Likelihood, Bayesian, and non-parametric bootstrapping) are consistent, indicating a high efficacy of the BNT162b2 vaccine.
