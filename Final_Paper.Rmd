...
### P-Value
Then, we did a likelihood ratio test on $\psi$. Notice that here our alternative hypothesis becomes $H_1:\psi_0\neq 0.3$ because  Plug in $\psi_0^{mle}=0.9506173$ and $t=8$, we know that the likelihood ratio test statistic $W$ equals to: $W = 2[\ell(\widehat{\psi_0^{mle}}) - \ell(\psi^{null})]=121.6012$.
...
```
<<<<<<< HEAD
Thus, using R, the P-value for the Likelihood Ratio Test statistic using the approximate chi square distribution is $P(W > 121.6012) = P(\psi > 0.3) = 2.822313e\times 10^{-28}$, which is very small.
=======
Thus, using R, the P-value for the Likelihood Ratio Test statistic using the approximate chi square distribution is $P(W > 121.6012) = 2.822313e\times 10^{-28}$, which is very small. So, we have enough evidence to reject the null hypothesis and we have $P(\psi > 0.3) = 1$ instead. Therefore, we can conclude that the vaccine efficacy is greater than 30\%.
>>>>>>> 669e4b68e59317fd389da18a7dccb7e28887db23
...
Since we used the simple quantile method without any assumptions, we calculated the empirical p-value. Here, since all of the samples' vaccine efficacy are greater than the $\psi_0=0.3$ in the null hypothesis and our alternative hypothesis is that $\psi_0>0.3$, then there is no chance of observing a test statistic as extreme or more extreme than the one observed under the null hypothesis, resulting in the empirical p-value to be 0. Therefore, it’s impossible that when $H_0$ is true, do we see the the vaccine efficacy or more extreme. Since 0 is smaller than any possible alpha value, we to reject the null hypothesis and conclude that the true $\psi_0>0.3$. Therefore, the vaccine efficacy of BNT162b2 is larger than 30%.
...
Maximum Likelihood (ConfI) & 0.9155621 & 0.9856725 & 2.822313×$10^{-28}$  \\ \hline
Bayesian prior Beta(0.700102, 1) (CredI) &  0.9035199 & 0.9762552 & 1.96x$10^{-28}$ \\ \hline
Bayesian Prior Beta(1, 1) (CredI) & 0.9009794 & 0.9750466 & 1.96x$10^{-28}$ \\ \hline
...
\end{table}
  ...
  In the Likelihood method, the vaccine efficacy in the combined maximum likelihood estimator is around 95.06\%. The corresponding 95\% confidence interval for the BNT162b2 vaccine efficacy is [0.9155621, 0.9856725]. W is ... The P-value for the Likelihood Ratio Test statistic using the approximate chi square distribution is $2.822313e\times 10^{-28}$, which is smaller than any common alpha. Thus, we have enough evidence to reject the null hypothesis and conclude that the true vaccine efficacy is greater than 30\%.
  
  Then, in the Bayesian analysis, based on the given information, the prior distribution: $Beta(0.700102, 1)$, we then can calculate the posterior distribution: $Beta(8.700102, 163)$. The 95\% Bayesian credible interval is [0.9035199, 0.9762552]. For $\psi$ represents the vaccine efficacy against COVID 19, the middle 95% of the posterior distribution of vaccine efficacy resulting from the prior is the range 90.3% to 97.6%. There is a 95% probability that $\psi_0$ in between 90.3% and 97.6%. Besides, we consider a non informative uniform prior distribution of $\pi$: $Beta(1,1)$. The posterior distribution is $Beta(9,163)$, and the 95\% Bayesian credible interval is [0.9009794, 0.9750466]. For $\psi$ represents the vaccine efficacy against COVID 19, the middle 95% of the posterior distribution of vaccine efficacy resulting from the prior is the range 90.3% to 97.6%. There is a 95% probability that $\psi_0$ in between 90.1% and 97.5%.
  
  For prior Beta(0.700102, 1), we calculated that the posterior probability $P(\psi_0 \leq 0.3|t)=1.96 \times 10^{-28}$. For prior Beta(1, 1), we calculated that the posterior probability $P(\psi_0 \leq 0.3|t)=3.73 \times 10^{-28}$. Since $3.73 \times 10^{-28} > 1.96 \times 10^{-28}$, then $1 - 3.73 \times 10^{-28} < 1 - 1.96 \times 10^{-28}$. Thus, $P(\psi_0 > 0.3|t)$ for prior Beta(0.700102, 1) is much closer to 1, so prior Beta(0.700102, 1) has greater evidence to reject the null hypothesis that vaccine efficacy is 30%. The probability that the true vaccine efficacy against COVID 19 is greater than 30% is approximately 100%. The evidence suggests that vaccine efficacy is greater than 30%.
  
  For prior Beta(1, 1), we calculated that the posterior probability $P(\psi_0 \leq 0.3|t)=3.73 \times 10^{-28}$. For prior Beta(0.700102, 1), we calculated that the posterior probability $P(\psi_0 \leq 0.3|t)=1.96 \times 10^{-28}$. Since $3.73 \times 10^{-28} > 1.96 \times 10^{-28}$, then $1 - 3.73 \times 10^{-28} < 1 - 1.96 \times 10^{-28}$. Thus, $P(\psi_0 > 0.3|t)$ for prior Beta(0.700102, 1) is much closer to 1, so prior Beta(0.700102, 1) has greater evidence to reject the null hypothesis that vaccine efficacy is 30%. The probability that the true vaccine efficacy against COVID 19 is greater than 30% is approximately 100%. The evidence suggests that vaccine efficacy is greater than 30%.
  
  For Non-parametric Bootstrapping, by generating 10000 bootstrapped estimation of $\psi_0$, the 95% simple bootstrap confidence interval is [0.9097410, 0.9819328]. The empirical p-value among the 10000 generated samples is 0, resulting that the bootstrapped vaccine efficacy is highly unlikely to have occurred under the assumption of the null hypothesis being true. Since 0 is smaller than any possible alpha value, we to reject the null hypothesis and conclude that the true vaccine efficacy of BNT162b2 is larger than 30%.
...
## 4.1.2 Representing $\psi$ using $\pi$ and representing $\pi$ using $\psi$
...
## 4.2 Detailed Calculation of the first derivative of $\ell(\psi)$
\begin{align*}
\frac{d}{d \psi} \ell(\psi) &= \frac{d}{d \psi} \left(ln\left(\binom{170}{t}\right) + t \cdot (ln(1 - \psi) - ln(2 - \psi)) + (t - 170) \cdot ln(2 - \psi)\right)=0 \\
...
## 4.2 Code for Likelihood
```{r ref.label="likelihood", echo = TRUE, eval = F}

```

## 4.3 Code for Bayesian
