...
>>>>>>> 9899287fcd492ff5a3f195bb7a44d2fa3f9288fd
...
CI_pi <- qbeta(p = c(0.025,0.975), shape1=t+prior_alpha, shape2=n-t+prior_beta)
CI_pi_lower <- CI_pi[1]
CI_pi_upper <- CI_pi[2]

prior_psi <- beta.select(quantile1 = list(p=0.5, x=0.948),
                         quantile2 = list(p=0.95, x=0.96559))
...
The median of $\pi$ is `r posterior_median_pi`. The median vaccine efficacy against Covid 19 is about 0.4878%, which indicates that 50% vaccine efficacy against Covid 19 is lower than 0.4878% and 50% vaccine efficacy against Covid 19 is higher than 0.4878%.


Prior: $g(\psi_0)$ = Beta(`r prior_psi_alpha`, `r prior_psi_beta`)

Posterior distribution: 

$h(\psi_0|t)=$ Beta(`r prior_psi_alpha`+8, 170-8+`r prior_psi_beta`)

```{r}
alpha_post <- prior_psi_alpha+8
alpha_post
beta_post <- 170-8+prior_psi_beta
beta_post
```

Therefore, $h(\psi_0|t)=$ Beta(`r alpha_post`, `r beta_post`)
...
```
```{r}
CI_psi <- qbeta(p = c(0.025,0.975), shape1=t+prior_psi_alpha, shape2=n-t+prior_psi_beta)
CI_psi
```

The 95\% Bayesian credible interval reported as [0.02319402, 0.08799074], which indicate that there is a 95% probability that the true value of $\psi_0$ falls within the interval [0.02319402, 0.08799074]. We are 95% confident that the true vaccine efficacy against Covid 19 is between approximately  and . It suggests that the true vaccine efficacy is very low and falls within a narrow interval.

Calculate posterior probability $P(\psi_0 > 0.3|t)$

```{r}
post_prob <- pbeta(q=0.3, shape1=t+prior_psi_alpha, shape2=n-t+prior_psi_beta, lower.tail = FALSE)
post_prob
```
